...
class RBB:
    ...
    def print(self):
        ...
        fileName = ".\\rbbresult\\" + self.duType + "\\" + self.ran + "_" + self.rbbName + ".txt"
...
RBB101AStandardList = ["W", "G", "L" ]

RUTypeRBB111A = ["RRUS01", "RRUS02", "RRUW01", "RRUW02", "RRUW03", "RUL01", "RUS01", "RUS02", "RUS03", "RUW01", "RUW02"]
RBB111AStandardList = ["W", "G", "L" ]

RUTypeRBB111C = ["RUS12mRBS6501radiopart", "mRRUS12", "mRRUS61", "Radio2203", "Radio2212", "Radio2217", "Radio2237", "Radio2218", "Radio2219", "RRU2208", "RRU2216",
...
RBB111CStandardList = ["W", "G", "L" ]


RBBListFor1RU = ["RBB101A", "RBB111A", "RBB111C"]
"""
RBBListFor1RU = ["RBB101A", "RBB111A", "RBB111C", "RBB111D", "RBB112A", "RBB112B", "RBB121A", "RBB122A", "RBB201A", "RBB201B", "RBB202B",
                 ...
                 "RBB242D", "RBB441D", "RBB441E", "RBB441F", "RBB442F", "RBB442G", "RBB442H", "RBB881A", "RBB882A"]
"""

for RBB in RBBListFor1RU:
    RUType = "RUType" + RBB
    standardList = RBB + "StandardList"
    RBB1RUMap[RBB] = (getattr(thisModule, RUType), getattr(thisModule, standardList), "RBBSNSMRWith1RU" )
 

for rbb in RBB1RUMap.keys():
    className = RBB1RUMap[rbb][2]
    ruTypeList = RBB1RUMap[rbb][0]
    standardList = RBB1RUMap[rbb][1]
...
for RBB in RBBListFor2RU:
    ...
    RBB2RUMap[RBB] = (getattr(thisModule, RU1Type), getattr(thisModule, RU2Type), getattr(thisModule, standardList), getattr(thisModule, RBBClass) )
 

for rbb in RBB2RUMap.keys():
    className = RBB2RUMap[rbb][3]
    ru1TypeList = RBB2RUMap[rbb][0]
    ru2TypeList = RBB2RUMap[rbb][1]
    standardList = RBB2RUMap[rbb][2]
    for ran in standardList:
        for duType in duTypeListMap[ran]:
            #find whether the rbb is allowed in this duType
            if duType in duTypeRbbMap.keys():
...
for RBB in RBBListFor3RU:
    ...
    RBBClass = RBB + "RBBClass"
    #RBB2RUMap["RBB221A"] = (RUTypeRBB221A, RUTypeRBB221A, RBB221AStandardList, "RBBSNSMRWith2RUCascadeAndCC" )
    RBB3RUMap[RBB] = (getattr(thisModule, RUType), getattr(thisModule, standardList), getattr(thisModule, RBBClass) )
 

for rbb in RBB3RUMap.keys():
    className = RBB3RUMap[rbb][2]
    ruTypeList = RBB3RUMap[rbb][0]
    standardList = RBB3RUMap[rbb][1]
    for ran in standardList:
        for duType in duTypeListMap[ran]:
            #find whether the rbb is allowed in this duType
            if duType in duTypeRbbMap.keys():
...
for RBB in RBBListFor4RU:
    ...
    standardList = RBB + "StandardList"
    RBBClass = RBB + "RBBClass"
    RBB4RUMap[RBB] = (getattr(thisModule, RUType), getattr(thisModule, standardList), getattr(thisModule, RBBClass) )
 

for rbb in RBB4RUMap.keys():
    className = RBB4RUMap[rbb][2]
    ruTypeList = RBB4RUMap[rbb][0]
    standardList = RBB4RUMap[rbb][1]
